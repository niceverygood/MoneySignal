...
## 남은 작업 (TODO)
- [ ] 토스페이먼츠 정기결제 가맹 심사 승인 대기 (03-06 신청)
- [ ] 심사 승인 후 빌링키 실결제 테스트
- [ ] 실결제 테스트 후 채널키 실연동으로 복원 (`channel-key-2886ee3a-377c-4b52-8c64-fc36f2ec587d`)
- [ ] 실서비스 E2E 시나리오 테스트 (체크리스트: ~/Desktop/MoneySignal_테스트_체크리스트.md)
- [x] 결제 방식 일반결제 → 빌링키(정기결제) 전환 (03-09 완료 — requestIssueBillingKey + /api/billing/issue)
- [x] 텔레그램 봇 생성 + 웹훅 등록 + /start 테스트 성공 (03-03 완료)
- [x] AI질문 페이지 React #310 에러 수정 (03-03 완료)
- [x] Anthropic API 크레딧 충전 (03-03 완료)
- [x] 알리고 서버 IP 등록 + 알림톡 실발송 테스트 (03-03 완료)
- [x] 알림톡 Supabase DB http extension 이전 + 회원가입 중복검사 (02-27 완료)
- [x] 알림톡 UF_6812 (시그널) 승인 완료 (02-26)
- [x] 코드 레벨 테스트 + 보안/UX 이슈 11건 수정 (02-26 완료 — DB에러노출방지 + CRON_SECRET제거 + 금액검증 등)
- [x] 알림톡 UF_6820 (TP) + UF_6821 (SL) 승인 완료 (02-25)
